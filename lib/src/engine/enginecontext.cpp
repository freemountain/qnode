...
EngineContext::EngineContext(INodeEngine *nodeEngine, ModuleLoader* loader, QJSEngine *jsEngine) : QObject(dynamic_cast<QObject *>(nodeEngine))
{
...
QJSValue EngineContext::wrapModule(QObject *module, QString wrapperPath, QJSValueList args) {
    QString wrapperSrc = Utils::readFile(wrapperPath);
    QJSValue jsMod = jsEngine->newQObject(module);

    //return JSValueUtils::wrapModule2(jsEngine, jsEngine->newQObject(module), wrapperSrc);
    ...
    wrapperArgs << jsMod << helper << jsCtx << JSValueUtils::listToArray(jsEngine, args);

    return wrapper.call(wrapperArgs);
}

QJSEngine EngineContext::getJsEngine() {
    return this->jsEngine;
}

INodeEngine EngineContext::getNodeEngine() {
