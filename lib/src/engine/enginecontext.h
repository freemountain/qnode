...
class EngineContext : public QObject
{
    Q_OBJECT
public:
    explicit EngineContext(INodeEngine *nodeEngine, ModuleLoader* loader, QJSEngine* jsEngine);

    QTextStream* getOutStream();
    QTextStream* getErrorStream();

    Q_INVOKABLE QJSValue require(QString module);
    QJSValue wrapModule(QObject* module, QString wrapperPath, QJSValueList args = QJSValueList());
