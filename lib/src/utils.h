...
inline QString methodName(const std::string& prettyFunction)
{
    ...
    QStringList token = QString::fromStdString(prettyFunction.substr(begin,end)).split("::");
...
#define TRACE(x) QTextStream(stdout) << "TRACE " << x << " " << __METHOD_NAME__ << " " << __FILE__ << ":" << __LINE__ << "\n"
#define TRACE_METHOD() QTextStream(stdout) << "TRACE " << __METHOD_NAME__ << " " << __FILE__ << ":" << __LINE__ << "\n"
