...
private Q_SLOTS:
    ...
    void run() {
        ...
        loop.connect(&nodeEngine, &NodeEngine::finished, [&loop, file, &msg, &failed](const QJSValue &result) {
            if(result.isError()) {
                QString name = result.property("constructor").property("name").toString();
                QString stack = result.property("stack").toString();
                QString message = result.property("message").toString();
                msg = QString("%1 failed \n%2 %3:\n%4").arg(file, name, message, stack);
                failed = true;
            }
            loop.quit();
        });

        connect(&nodeEngine, &NodeEngine::ipcMessage, [&ipcMessages](const QJSValue &msg){
            ipcMessages << msg;
        });
        ...
        if(!failed && testIpc) {
            ...
            QString b = JSValueUtils::stringify(JSValueUtils::listToArray(nodeEngine.getEngine(), ipcMessages));
            QCOMPARE(a, b);
        }
    }
};
